...
export async function GET(req: NextRequest) {
  ...
  const items = await prisma.saleItem.findMany({
    ...
    select: { offerId: true, nameSnapshot: true, quantity: true, lineTotal: true },
  });

  const agg = new Map<
    string,
    { offerId: string | null; name: string; quantity: number; revenueM: number }
  >();
  for (const it of items) {
    const key = it.offerId ?? `__deleted_${it.nameSnapshot}`;
